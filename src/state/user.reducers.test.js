...
describe('(Reducer) User state', () => {
      ...
      it('(setUserData) Updates the details of a user', () => {
            const newState = user.reducers.setUserData( initialState, {
                  tokens: 8,
                  votingRecord: {'Petro Poroshenko' : 1, 'Khalifa Bin Zayed': 20}
            });
      ...
      it('(updateVotingRecord) Updates its voting record on vote event', async () => {
            const newState = user.reducers.updateVotingRecord( initialState, 'Petro Poroshenko');
            expect(newState).toEqual({
                  tokens: 9,
                  votingRecord: {'Petro Poroshenko' : 11, 'Asif Ali Zardari': 2}
            });
      });
